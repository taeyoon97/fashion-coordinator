...
export async function generateOutfitImage(
  ...
  try {
    ...
    console.error("이미지 생성 상세 에러:", error);
    const errorMessage = error?.message || String(error);
    
    // 할당량 초과나 지원하지 않는 지역일 경우 대체 이미지 반환
    if (errorMessage.includes("429") || errorMessage.includes("RESOURCE_EXHAUSTED") || errorMessage.includes("User location is not supported")) {
      console.warn("이미지 생성 할당량 초과 또는 미지원 지역입니다. 샘플 이미지를 표시합니다.");
      return `https://picsum.photos/seed/${encodeURIComponent(clothingDescription.slice(0, 10))}/800/1200`;
    }
