...
export default function App() {
  ...
  const [classicImageError, setClassicImageError] = useState<string | null>(null);
  const [trendyImageError, setTrendyImageError] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTrendyImage(null);
    setClassicImageError(null);
    setTrendyImageError(null);
    ...
    try {
      ...
      // Generate images sequentially or separately to handle errors better
      try {
        const cImg = await generateOutfitImage(classicDesc, condition);
        setClassicImage(cImg);
      } catch (cErr: any) {
        console.error("Classic image error:", cErr);
        setClassicImageError(cErr.message);
      }

      try {
        const tImg = await generateOutfitImage(trendyDesc, condition);
        setTrendyImage(tImg);
      } catch (tErr: any) {
        console.error("Trendy image error:", tErr);
        setTrendyImageError(tErr.message);
      }
    } catch (err) {
      console.error(err);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/50 focus:ring-2 focus:ring-brand-500 outline-none transition-all text-sm appearance-none"
                  >
                    <option value="">스타일을 선택해주세요 (선택 안 함)</option>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-white/80 backdrop-blur-md rounded-3xl overflow-hidden border border-white/40 shadow-lg aspect-[4/5] relative group">
                        {classicImage ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 bg-slate-50/50">
                            <AlertTriangle className="mb-2 text-amber-500" size={32} />
                            <p className="text-sm font-bold text-slate-600 mb-1">이미지 생성 불가</p>
                            <p className="text-[10px] leading-tight break-keep">{classicImageError}</p>
                          </div>
                        ) : (
                      ...
                      <div className="bg-white/80 backdrop-blur-md rounded-3xl overflow-hidden border border-white/40 shadow-lg aspect-[4/5] relative group">
                        {trendyImage ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 bg-slate-50/50">
                            <AlertTriangle className="mb-2 text-amber-500" size={32} />
                            <p className="text-sm font-bold text-slate-600 mb-1">이미지 생성 불가</p>
                            <p className="text-[10px] leading-tight break-keep">{trendyImageError}</p>
                          </div>
                        ) : (
                          <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 bg-slate-50/50">
                            <Loader2 className="animate-spin mb-2" size={24} />
